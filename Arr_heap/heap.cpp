...
template <typename E, typename Comp> class heap 
{
	private:
		...
		void siftdown(int pos) // Helper function to put element in its correct place 
		{
	...
	public:
		...
		void insert(const E& it) // Insert "it" into the heap
		{
		...
		E removefirst() // Remove first value 
		{
		...
		E remove(int pos) // Remove and return element at specified position
		{
